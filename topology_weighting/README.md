...
* Now **open a Python interactive session**, and also open a script in a text editor, because we are going to modify rerun some of these lines multiple times
...
```
Can you tell what is different between the trees?
...
ts.num_trees
```
* and, if we sare, we can look at the first tree in the tree sequence:
...
```python
weightsData = twisst.weightTrees(ts.trees(), taxa, treeFormat="ts", verbose=True)
```

* For plotting we also want to record the start and end positions of each tree on the chromosome. We will extract these ourselves and write them to a file

```python
with gzip.open("msprime.4pop.data.tsv.gz", "wt") as dataFile:
...
```R
weights_file <- 'msprime.4pop.weights.tsv.gz'

window_data_file <- 'msprime.4pop.data.tsv.gz'
